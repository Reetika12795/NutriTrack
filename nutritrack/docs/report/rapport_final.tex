...
% ---- NutriTrack logo ----
\includegraphics[width=0.75\textwidth]{../assets/logos/nutri_track_logo_v8.pdf}

\vspace{0.4cm}

% ---- Title band (colored bar with title) ----
\begin{tikzpicture}
  \node[fill=ntgreen, text=white, minimum width=0.95\textwidth,
        minimum height=1.1cm, rounded corners=4pt, align=center,
        inner sep=6pt]
...
% ---- Subtitle lines ----
{\large\color{ntdarkgreen}\bfseries Expert in Massive Data Infrastructures}\\[0.1cm]
{\normalsize\color{ntgray} RNCP37638 \textbullet\ Level 7 (Master's equivalent)}

\vspace{0.7cm}

% ---- Centered plain-text author info ----
{\normalsize
\textbf{\color{ntdarkgreen}Name:} Reetika Gautam \quad\textbullet\quad
\textbf{\color{ntdarkgreen}Organization:} Simplon \quad\textbullet\quad
...
\begin{tikzpicture}
  \node[fill=ntgreen!10, draw=ntgreen!40, line width=0.5pt,
        ...
        minimum height=3.6cm, align=center, inner sep=10pt] {
    \begin{tabular}{c}
      {\small\bfseries\color{ntdarkgreen} Powered by}\\[0.25cm]
      \begin{tabular}{c @{\hspace{2cm}} c}
        \includegraphics[height=2.2cm]{../assets/logos/Simplon-HD.png} &
        \includegraphics[height=2.2cm]{../assets/logos/PublicisGroupelogo-BlackText-300x300.png} \\
      \end{tabular}
    \end{tabular}
  };
\end{tikzpicture}
...
% ---- Footer caption band ----
\begin{tikzpicture}
  \node[fill=ntblue!10, draw=ntblue!40, line width=0.5pt,
        rounded corners=4pt, text width=0.9\textwidth,
        align=center, inner sep=8pt,
        font=\footnotesize\itshape\color{ntblue}]
    {This document covers all 21 competencies (C1--C21)
...
\node[ext, right=1cm of off] (parquet) {OFF Parquet\\(HuggingFace)};
...
\node[service=ntblue, below left=1cm and 0.6cm of airflow] (postgres)
  {PostgreSQL 16\\{\tiny :5432}\\{\tiny app | dw | raw | staging}};
...
% --- Row 4: Applications ---
\node[service=ntgreen, below=1.2cm of postgres] (fastapi)
  {FastAPI\\{\tiny :8000}\\{\tiny REST API + JWT}};
\node[service=ntgreen, below=1.2cm of minio] (streamlit)
  {Streamlit\\{\tiny :8501}\\{\tiny 4-role Frontend}};
...
\midrule
User (patient) & Own meals only & None & None & 6 pages (login, search, log, dashboard, alternatives, compare) \\
Nutritionist & Read all meals of consenting patients & None & None & User dashboard + trends \\
Analyst & Read products only & Read datamarts (pseudonymized) & Read gold bucket & Product analytics + catalogue \\
Admin & Full & Full & Full (bronze/silver/gold/backups) & All pages + admin UI \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
1 & Incomplete requirements & Medium & High & Interview grids with all stakeholder groups; iterative validation with Sophie \\
2 & API rate limiting / data unavailability & Medium & High & Multi-source strategy (API + Parquet + scraping); local caching \\
2 & Data-quality issues in OFF & High & Medium & 7-rule PySpark cleaning; 6 data-quality checks; cleaning report \\
3 & Schema evolution breaking API & Low & High & Versioned API (\texttt{/api/v1/}); migration scripts \\
4 & ETL failures blocking analytics & Medium & High & Airflow retries (2--3 attempts); Mailhog alerting; SLA dashboard \\
...
5 & MinIO storage exhaustion & Low & Medium & Lifecycle rules (bronze 90\,d, backups 30\,d); storage monitoring \\
...
site (see Section~\ref{sec:cicd-docs}).
...
deck. See Chapter~\ref{ch:cicd} for the full list of
documentation deliverables and the slide deck used for the
defense.
...
\nutritrack{} started as a simple question from Sophie Yang: \textit{``I spend
hours looking up products. I need a platform where my patients log meals and I
see the analytics.''} Six phases and twelve weeks later, that one sentence has
become a running platform that covers all 21~competencies
(\competence{C1}--\competence{C21}) of the
\textit{Expert in Massive Data Infrastructures} certification.

Along the way, this project transformed a public dataset of 798\,177 French
food products into 777\,116 validated records through a distributed PySpark
pipeline, loaded them into a star-schema data warehouse, mirrored them into a
bronze/silver/gold data lake, and exposed them through an async REST API
backed by a four-role Streamlit front-end. Everything runs on a single
\texttt{docker compose} command, every script is versioned on Git, and every
architectural choice --- from PostgreSQL for \rgpd{}-compliant row-level
delete, to MinIO for self-hosted S3-compatible storage, to Airflow for
dependable orchestration --- ties back to a specific need Sophie expressed
during the discovery interviews.

What makes this project meaningful goes beyond the technical stack. It
demonstrates that a small practice like Sophie's can run a production-grade
data platform with zero licensing cost and less than one~hundred euros per
year of operating cost, thanks to a thoughtfully chosen open-source stack.
It demonstrates that \rgpd{} compliance is not an afterthought but an
architectural driver: personal data stays in PostgreSQL where it can be
deleted row-by-row, and public data flows to the Parquet lake. And it
demonstrates the power of starting from the client, not from the technology
--- every feature in this report earns its place because it answers one of
Sophie's five pain points.

\nutritrack{} is more than a certification deliverable: it is a complete,
reproducible, documented data platform that a real nutritionist could deploy
tomorrow. The code is public, the report you are holding explains every
choice, and the oral defense will show the platform running in real time.
...
I would like to take a moment to thank the two organisations that
made this project --- and the certification behind it --- possible.

\vspace{0.6cm}

\noindent\textbf{To Simplon, my school.} Thank you for designing a
curriculum that pushes us to build real data platforms end-to-end
rather than toy examples. The twelve months I spent with your
instructors and fellow learners gave me the technical foundations
and the confidence to tackle a project of this scale. The
referential you walked us through week after week is exactly what
helped me structure this report and the \nutritrack{} platform
around the 21~competencies of \textit{RNCP37638}. Thank you to
every instructor, every mentor and every classmate who reviewed my
work and challenged my choices.

\vspace{0.4cm}

\noindent\textbf{To Publicis Groupe, my company.} Thank you for
trusting me with the time, the context and the professional
environment that turned this certification from an academic
exercise into a workplace-ready project. The daily exposure to
large-scale data systems, code reviews, and cross-functional
discussions shaped every architectural decision in \nutritrack{},
from the choice of PySpark for cleaning to the medallion data lake
on MinIO. Thank you for believing that investing in my learning
would benefit not only me but also the teams I work with.

\vspace{0.6cm}

\noindent Building \nutritrack{} was, above all, a privilege.
Sophie Yang may be a fictional client but the problems she
represents are real, the technical stack is real, the code runs,
and every line of this report reflects something I learned by
doing. I am grateful to both Simplon and Publicis for giving me
the opportunity to learn by building something that matters.
