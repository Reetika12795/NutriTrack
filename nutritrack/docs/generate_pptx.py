...
def add_bullet_slide(slide, left, top, width, height, bullets, font_size=16,
                     color=WHITE, level_colors=None, font_name="Calibri"):
    txBox = slide.shapes.add_textbox(Inches(left), Inches(top),
                                      Inches(width), Inches(height))
    tf = txBox.text_frame
    tf.word_wrap = True
...
def add_content_slide(title, accent_color=ACCENT_BLUE, notes_text=None):
    ...
    shape2 = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE,
                                     Inches(0.5), Inches(1.0), Inches(2.5), Inches(0.04))
    shape2.fill.solid()
...
def add_table_to_slide(slide, left, top, width, height, headers, rows,
                       header_color=ACCENT_BLUE, font_size=12):
    cols = len(headers)
    row_count = len(rows) + 1
    table_shape = slide.shapes.add_table(row_count, cols,
...
add_textbox(slide, 1, 3.8, 11, 0.6,
            "Expert en infrastructures de donnees massives (Data Engineer)",
            20, False, LIGHT_GRAY, PP_ALIGN.CENTER)
add_textbox(slide, 1, 4.4, 11, 0.5, "RNCP37638 - Level 7 (Master's equivalent)",
            18, False, LIGHT_GRAY, PP_ALIGN.CENTER)
add_textbox(slide, 1, 5.5, 11, 0.5, "[Candidate Name]  |  March 2026",
            18, False, MEDIUM_GRAY, PP_ALIGN.CENTER)

# =====================================================================
#  AGENDA SLIDE
# =====================================================================
slide = add_content_slide("Defense Agenda - 90 Minutes", ACCENT_BLUE)
add_table_to_slide(slide, 0.8, 1.4, 11.5, 4.5,
...
add_textbox(slide, 0.8, 6.3, 11, 0.5,
...
add_section_divider(
    ...
    BLOCK1_COLOR, "E1 - Case Study", "5 minutes | Block 1"
)

# E1 - Context
slide = add_content_slide("E1 | Project Context & Business Need", BLOCK1_COLOR)
add_bullet_slide(slide, 0.8, 1.3, 11, 5.5, [
    ("Business Need", 0),
...
add_section_divider(
    ...
    BLOCK1_COLOR, "E2 - Professional Situation", "20 minutes | Block 1"
)

# E2 - Data Topography: Semantics
slide = add_content_slide("E2 | C2 - Data Topography: Business Glossary", BLOCK1_COLOR)
add_table_to_slide(slide, 0.5, 1.3, 12, 5.0,
    ["Business Object", "Description", "Key Metadata"],
    [
        ["Product", "Food product identified by barcode", "barcode, name, brand, category, nutrition/100g, Nutri-Score, NOVA"],
...
add_table_to_slide(slide, 0.8, 1.3, 11.5, 2.5,
...
add_bullet_slide(slide, 0.8, 4.8, 11, 2.0, [
...
add_table_to_slide(slide, 0.3, 1.3, 12.5, 5.0,
...
add_table_to_slide(slide, 0.5, 1.3, 12, 4.0,
...
add_table_to_slide(slide, 0.5, 1.3, 12, 5.0,
    ...
    [
        ["DB for OLTP/OLAP", "Single PostgreSQL (separate schemas)", "Separate instances", "Simpler deployment; adequate for current scale"],
...
add_table_to_slide(slide, 0.8, 1.3, 11.5, 3.5,
...
add_table_to_slide(slide, 0.5, 1.3, 12, 3.5,
    ["Data Category", "Legal Basis", "Retention", "Security Measures"],
    [
        ["User account (email, username, hash)", "Consent Art. 6.1.a", "3 years after last login", "bcrypt hashing, encrypted storage"],
        ["User profile (age, activity, goal)", "Consent Art. 6.1.a", "3 years or consent withdrawal", "SHA256 pseudonymized in analytics"],
        ["Meal tracking (logs, history)", "Consent Art. 6.1.a", "2 years after creation", "UUID association, deletable on request"],
        ["Product data (Open Food Facts)", "Legitimate interest Art. 6.1.f", "Indefinite (public data)", "Public data, no personal info"],
    ], BLOCK1_COLOR, 12)
add_bullet_slide(slide, 0.8, 5.2, 11, 2.0, [
...
], color=WHITE, level_colors=[ACCENT_ORANGE, LIGHT_GRAY])
...
add_section_divider(
    ...
    BLOCK1_COLOR, "E3 - Role Play / Kickoff Meeting", "10 minutes | Block 1"
)
...
add_table_to_slide(slide, 0.5, 1.3, 12, 4.5,
...
add_bullet_slide(slide, 0.8, 1.3, 5, 5.5, [
...
], color=WHITE, level_colors=[ACCENT_BLUE, LIGHT_GRAY], font_size=15)
add_bullet_slide(slide, 6.5, 1.3, 6, 5.5, [
    ("Sprint Rituals", 0),
...
add_bullet_slide(slide, 0.8, 5.5, 11, 1.5, [
...
add_section_divider(
    ...
    BLOCK2_COLOR, "E4 - Professional Situation", "15 minutes | Block 2"
)
...
add_bullet_slide(slide, 0.8, 5.2, 11, 2.0, [
    ("All scripts follow C8 structure: entry point, dependency init, external connections, logic rules, error handling, result saving", 0),
...
], color=WHITE, level_colors=[ACCENT_GREEN, LIGHT_GRAY])
...
add_table_to_slide(slide, 0.3, 1.3, 12.5, 5.0,
    ...
    [
        ...
        ["Q2", "Full-text product search + Nutri-Score filter", "GIN index on to_tsvector", "ts_rank + plainto_tsquery"],
        ["Q3", "Healthier alternatives in same category", "Window function avoids self-join", "ROW_NUMBER PARTITION BY"],
...
add_section_divider(
    ...
    BLOCK3_COLOR, "E5 - Professional Situation", "10 minutes | Block 3"
)
...
add_table_to_slide(slide, 0.5, 1.3, 5.5, 3.5,
...
add_table_to_slide(slide, 6.5, 1.3, 6, 2.5,
...
add_textbox(slide, 0.5, 5.0, 12, 0.8,
...
], color=WHITE, level_colors=[BLOCK3_COLOR, LIGHT_GRAY])
...
add_section_divider(
    ...
    BLOCK3_COLOR, "E6 - Case Study (Q&A)", "10 minutes Q&A | Block 3"
)
...
add_bullet_slide(slide, 0.8, 1.3, 11, 3.0, [
...
add_bullet_slide(slide, 0.8, 4.5, 11, 3.0, [
...
add_section_divider(
    ...
    BLOCK4_COLOR, "E7 - Professional Situation", "10 minutes | Block 4"
)
...
add_table_to_slide(slide, 0.8, 1.3, 11.5, 2.5,
    ...
    [
        ["Volume", "3M+ products in OFF dataset (~5 GB)", "DuckDB reads Parquet without memory loading; MinIO scales to PB"],
...
add_table_to_slide(slide, 0.8, 4.8, 11.5, 2.0,
...
], color=WHITE, level_colors=[BLOCK4_COLOR, LIGHT_GRAY])
...
add_table_to_slide(slide, 0.2, 1.2, 12.8, 5.5,
...
add_textbox(slide, 1, 5.0, 11, 0.5, "NutriTrack | RNCP37638 Data Engineer Certification",
            18, False, LIGHT_GRAY, PP_ALIGN.CENTER)
add_textbox(slide, 1, 5.6, 11, 0.5,
            "Git Repository  |  docs/rapport_final.md  |  docker compose up -d",
            16, False, MEDIUM_GRAY, PP_ALIGN.CENTER)
