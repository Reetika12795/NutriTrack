...
| Constraint | Response |
|-----------|----------|
| Budget: ~48K EUR engineering + <100 EUR/yr OPEX | Open-source stack, zero licensing |
...
| Object Store | MinIO | AWS S3 | HDFS | Self-hosted, S3-compatible API, zero licensing |
...
### Engineering CAPEX: ~48,300 EUR (one-time)

Sophie paid for a team of engineers over 3 months to build the platform:

| Role | Duration | Rate | Cost |
|------|----------|------|------|
| Data Engineer (lead) | 3 months FT | 550 EUR/day | ~36,300 EUR |
| Data Architect | 6 weeks PT | 550 EUR/day | ~8,250 EUR |
| DevOps / CI-CD | 3 weeks PT | 500 EUR/day | ~3,750 EUR |
| **Total engineering** | | | **~48,300 EUR** |

This is a **one-time investment**. Once delivered, Sophie operates the platform herself.

### Infrastructure OPEX: < 100 EUR/year

| Item | Cost | Notes |
|------|------|-------|
| Domain name (optional) | ~15 EUR/year | For public-facing documentation |
| Electricity (local server) | ~80 EUR/year | Docker on existing hardware |
| Software licensing | 0 EUR | 100% open-source |
| **Total** | **< 100 EUR/year** | |

!!! info "Cloud Comparison"
    An equivalent AWS deployment (RDS + S3 + MWAA + CloudWatch) would cost approximately **~8,000 EUR/year**. The self-hosted Docker approach saves Sophie that entire amount — her only recurring cost is under 100 EUR/year.
